<?php

namespace App\Http\Middleware;

use Illuminate\Foundation\Inspiring;
use Illuminate\Http\Request;
use Inertia\Middleware;
use Spatie\Permission\Models\Permission;
use Tighten\Ziggy\Ziggy;

class HandleInertiaRequests extends Middleware
{
    /**
     * The root template that's loaded on the first page visit.
     *
     * @see https://inertiajs.com/server-side-setup#root-template
     *
     * @var string
     */
    protected $rootView = 'app';

    /**
     * Determines the current asset version.
     *
     * @see https://inertiajs.com/asset-versioning
     */
    public function version(Request $request): ?string
    {
        return parent::version($request);
    }

    /**
     * Define the props that are shared by default.
     *
     * @see https://inertiajs.com/shared-data
     *
     * @return array<string, mixed>
     */
    public function share(Request $request): array
    {
        [$message, $author] = str(Inspiring::quotes()->random())->explode('-');

        return [
            ...parent::share($request),
            'name' => config('app.name'),
            'appLogo' => asset('images/logo_humusense_colore.svg'),
            'coverPhoto' => asset('images/benvenuto.jpg'),
            'quote' => ['message' => trim($message), 'author' => trim($author)],
            'auth' => [
                'user' => $request->user(),
                'canImpersonate' => auth()->user()?->can('impersonate_user') ?? false,
                'isImpersonating' => session()->has('impersonated_by'),
                'permissions' => $request->user()?->getPermissionsViaRoles()->pluck('name'),
                'can' => $request->user()?->getPermissionsViaRoles()
                    ->map(function (Permission $permission): array {
                        return [$permission['name'] => auth()->user()->can($permission->name)];
                    })
                    ->collapse()
                    ->all(),
                'isSuperAdmin' => $request->user()?->isSuperAdmin() ?? false,
                'unreadNotifications' => $request->user()?->unreadNotifications->count(),
            ],
            'ziggy' => [
                ...(new Ziggy)->toArray(),
                'location' => $request->url(),
            ],
            'sidebarOpen' => ! $request->hasCookie('sidebar_state') || $request->cookie('sidebar_state') === 'true',
            'mapBox' => [
                'accessToken' => config('services.mapbox.access_token'),
            ],
            'flash' => [
                'success' => fn () => $request->session()->get('success'),
                'error' => fn () => $request->session()->get('error'),
                'warning' => fn () => $request->session()->get('warning'),
                'info' => fn () => $request->session()->get('info'),
            ],
            'appLocale' => $request->session()->get('locale', app()->getLocale()),
        ];
    }
}
