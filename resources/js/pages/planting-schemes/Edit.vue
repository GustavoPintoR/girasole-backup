...
const breadcrumbs = [
    ...
    { title: trans('ui.edit'), href: route('planting-schemes.edit', props.plantingScheme.id) }
];

const form = useForm({
    name: props.plantingScheme.name,
    pattern: props.plantingScheme.pattern,
    row_spacing: props.plantingScheme.row_spacing,
    plant_spacing: props.plantingScheme.plant_spacing,
    description: props.plantingScheme.description
});

function submit() {
    form.put(route('planting-schemes.update', props.plantingScheme.id), {
        preserveScroll: true,
        onSuccess: () => {
            //
        },
...
<template>
    <Head :title="trans('ui.planting_scheme_edit_header')" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">
                    {{ trans('ui.planting_scheme_edit_header') }}
                </h1>
            </div>

            <Card class="w-full">
                <CardHeader>
                    <CardTitle>{{ trans('ui.planting_scheme_edit_header_details') }}</CardTitle>
                    <CardDescription>{{ trans('ui.planting_scheme_edit_edit_desc') }}</CardDescription>
                </CardHeader>
                <CardContent>
                ...
                <CardFooter class="flex justify-end space-x-2">
                    ...
                    <Button type="submit" :disabled="form.processing" @click="submit">
                        {{ form.processing ? trans('ui.updating') : trans('ui.edit_planting_scheme') }}
                    </Button>
                </CardFooter>
            </Card>
        </div>
