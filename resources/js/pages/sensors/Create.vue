...
import { Company, SensorType, User } from '@/types';
...
watch(() => form.owner_id, async (newVal) => {
    ...
    // reset company selection when owner changes
    form.company_id = '';
    if (newVal) {
        try {
            const response = await axios.get('/api/cadastral-groups', {
                params: { user_id: newVal }
            });
        ...
        cadastralGroups.value = [];
        // Clear coordinates when no owner is selected
        form.latitude = '';
        form.longitude = '';
    }
});
...
<template>
    ...
    <Head :title="trans('ui.create_sensor')" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6 space-y-6">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">
                {{ trans('ui.create_sensor') }}
            </h1>
