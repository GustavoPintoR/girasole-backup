...
function deleteItem() {
    if (selectedId.value) {
        router.delete(route('sensor-operations.destroy', selectedId.value), {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem v-if="can.read_sensor_operation" class="p-0">
                <Link :href="route('sensor-operations.show', sensorOperation.id)" class="w-full">
            ...
            <DropdownMenuItem v-if="can.update_sensor_operation" class="p-0">
                <Link :href="route('sensor-operations.edit', sensorOperation.id)" class="w-full">
            ...
            <Button
                v-if="can.delete_sensor_operation"
                variant="ghost"
                size="sm"
                :title="trans('ui.delete')"
                @click="confirmDelete(sensorOperation.id)"
