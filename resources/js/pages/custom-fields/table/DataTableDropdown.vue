...
import { MoreHorizontal, Trash, Edit } from 'lucide-vue-next'
...
function deleteItem() {
    if (selectedId.value) {
        router.delete(route('custom-fields.destroy', selectedId.value), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem class="p-0">
                <Link :href="route('custom-fields.edit', props.field.id)" class="w-full">
            ...
            <DropdownMenuItem class="p-0">
                <Button variant="ghost" size="sm" :title="trans('ui.delete')" @click="confirmDelete(props.field.id)"
    ...
    <AlertDialog v-model:open="deleteDialogOpen">
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    {{ trans('ui.custom_field_delete_warning') || 'This will permanently delete the custom field and all its associated data. This action cannot be undone.' }}
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
