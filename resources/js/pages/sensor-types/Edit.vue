...
import { SensorOperation, SensorType } from '@/types';
...
const breadcrumbs = [
    { title: trans('ui.sensor_type'), href: route('sensor-types.index') },
    { title: trans('ui.edit'), href: route('sensor-types.edit', props.sensorType.id) },
];

const form = useForm({
    name: props.sensorType.name,
    description: props.sensorType.description,
    operation_ids: props.selectedOperationsIds || [],
});
...
function submit() {
    form.put(route('sensor-types.update', props.sensorType.id), {
        preserveScroll: true,
        onSuccess: () => {
            //
        },
...
<template>
    <Head :title="trans('ui.sensor_type_edit_header')" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">
                    {{ trans('ui.sensor_type_edit_header') }}
                </h1>
            </div>

            <Card class="max-w-4xl">
                <CardHeader>
                    <CardTitle>{{ trans('ui.sensor_type_edit_header_details') }}</CardTitle>
                    <CardDescription>{{ trans('ui.sensor_type_edit_edit_desc') }}</CardDescription>
                </CardHeader>
                <CardContent>
                ...
                <CardFooter class="flex justify-end space-x-2">
                    ...
                    <Button type="submit" :disabled="form.processing" @click="submit">
                        {{ form.processing ? trans('ui.updating') : trans('ui.edit_sensor_type') }}
                    </Button>
                </CardFooter>
            </Card>
        </div>
