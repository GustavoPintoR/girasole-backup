...
function deleteItem() {
    if (selectedId.value) {
        router.delete(route('roles.destroy', selectedId.value), {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem v-if="can.read_role" class="p-0">
                <Link :href="route('roles.show', role.id)" class="w-full">
            ...
            <DropdownMenuItem v-if="can.update_role" class="p-0">
                <Link :href="route('roles.edit', role.id)" class="w-full">
            ...
            <Button v-if="can.delete_role" variant="ghost" size="sm" :title="trans('ui.delete')" @click="confirmDelete(role.id)"
