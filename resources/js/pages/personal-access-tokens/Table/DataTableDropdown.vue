...
import { Eye, MoreHorizontal, Trash } from 'lucide-vue-next'
...
function deleteItem() {
    if (selectedId.value) {
        router.delete(route('api-keys.destroy', selectedId.value), {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem v-if="can.read_personal_access_token" class="p-0">
                <Link :href="route('api-keys.show', token.id)" class="w-full">
...
<!--            <DropdownMenuItem v-if="can.update_personal_access_token" class="p-0">-->
<!--                <Link :href="route('api-keys.edit', token.id)" class="w-full">-->
...
<!--            </DropdownMenuItem>-->
            ...
            <Button v-if="can.delete_personal_access_token" variant="ghost" size="sm" :title="trans('ui.delete')" @click="confirmDelete(token.id)"
