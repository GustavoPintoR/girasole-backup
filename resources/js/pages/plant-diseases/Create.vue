...
const breadcrumbs = [
    ...
    { title: trans('ui.create'), href: route('plant-diseases.create') },
];

const form = useForm({
    name: '',
    description: '' as string | null,
    cultivation_ids: [] as number[],
});
...
const submit = () => {
    form.post(route('plant-diseases.store'), {
        preserveScroll: true,
    });
};
...
<template>
    <Head :title="trans('ui.plant_disease_create_header')" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">
                    {{ trans('ui.plant_disease_create_header') }}
                </h1>
            </div>

            <Card class="max-w-4xl">
                <CardHeader>
                    <CardTitle>{{ trans('ui.plant_disease_details') }}</CardTitle>
                    <CardDescription>
                        {{ trans('ui.plant_disease_create_desc') }}
                    </CardDescription>
                </CardHeader>
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="flex items-center justify-end space-x-2 pt-4">
                            ...
                            <Button type="submit" :disabled="form.processing">
                                {{ form.processing ? trans('ui.creating') : trans('ui.create_plant_disease') }}
                            </Button>
                        </div>
                    </form>
                </CardContent>
