...
type PlantDisease = { id: number; name: string; description?: string | null };
...
function deleteItem() {
    if (selectedId.value) {
        router.delete(route('plant-diseases.destroy', selectedId.value), {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem v-if="can.read_plant_disease" class="p-0">
                <Link :href="route('plant-diseases.show', plantDisease.id)" class="w-full">
            ...
            <DropdownMenuItem v-if="can.update_plant_disease" class="p-0">
                <Link :href="route('plant-diseases.edit', plantDisease.id)" class="w-full">
            ...
            <Button
                v-if="can.delete_plant_disease"
                variant="ghost"
                size="sm"
                :title="trans('ui.delete')"
                @click="confirmDelete(plantDisease.id)"
