...
import { CalendarIcon, AlertCircle, InfoIcon } from 'lucide-vue-next';
...
const breadcrumbs = [
    ...
    {
        title: trans('ui.edit'),
        href: route('terms-and-conditions.edit', props.terms.id),
    },
];

const calendarDate = ref<CalendarDate>();
if (props.terms.active_at) {
    const dateParts = props.terms.active_at.split('-');
...
const form = useForm({
    ...
    summary: props.terms.summary,
    is_active: props.terms.is_active,
    active_at: props.terms.active_at,
});
...
const submit = () => {
    form.put(route('terms-and-conditions.update', props.terms.id), {
        preserveScroll: true,
        onSuccess: () => {
            //
        },
...
const wasActive = props.terms.is_active;
</script>

<template>

    <Head :title="trans('ui.terms_edit_header')" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">
                    {{ trans('ui.terms_edit_header') }}
                </h1>
            </div>

            <!-- Status info -->
            <Alert v-if="wasActive"
                class="mb-4 border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20 max-w-4xl">
            ...
            <Card class="max-w-4xl">
                <CardHeader>
                    <CardTitle>{{ trans('ui.terms_edit_header_details') }}</CardTitle>
                    <CardDescription>
                        {{ trans('ui.terms_edit_desc') }} {{ props.terms.version }}.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <!-- <Textarea id="description" v-model="form.description"
                                placeholder="Enter the full terms and conditions text..." :rows="10"
                                :class="{ 'border-red-500': form.errors.description }" :disabled="form.processing" /> -->
                            <WysiwygEditor v-model="form.description" :rows="10"/>
                        ...
                        <div class="space-y-2">
                            ...
                            <Textarea id="summary" v-model="form.summary"
                                placeholder="Brief summary of the terms and conditions changes..." :rows="3"
                        ...
                        <!-- Active Status -->
                        <div class="space-y-2" v-if="!wasActive">
                        ...
                        <!-- Warning Alert -->
                        <Alert v-if="form.is_active && !wasActive"
                    ...
                    <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
                                {{ format(parseISO(props.terms.created_at), 'PPP p') }}
                            </dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ trans('ui.last_updated') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
                                {{ format(parseISO(props.terms.updated_at), 'PPP p') }}
                            </dd>
                        </div>
                        <div v-if="props.terms.active_at">
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ trans('ui.active_since') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
